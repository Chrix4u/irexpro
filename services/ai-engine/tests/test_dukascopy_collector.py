...
def test_collection_recovers_transient_hour_without_silent_gap(
    ...
    assert result["serial_recovery_calls"] == 1
    assert attempts[datetime(2026, 1, 5, 10, tzinfo=UTC)] == 2
    assert len(pd.read_csv(output)) == 250



def test_collection_uses_bounded_multi_round_serial_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    attempts: dict[datetime, int] = {}
    ...
    def stubborn_fetch_hour(
        *,
        ...
        # must wait for the bounded second recovery round and then continue.
        if hour.hour == 10 and attempts[hour] < 3:
            raise RuntimeError("synthetic sustained transient 503")
    ...
    failed_hour = datetime(2026, 1, 5, 10, tzinfo=UTC)
    assert attempts[failed_hour] == 3
    assert sleeps == [15.0]
    assert result["recovered_hours"] == 1
    assert result["serial_recovery_calls"] == 2
    assert len(pd.read_csv(output)) == 250
