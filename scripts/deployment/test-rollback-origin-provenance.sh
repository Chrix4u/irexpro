...
expect_rollback_origin_rejection() {
  ...
  # If rollback reached git fetch, these deliberately non-local/unapproved
  # URLs could fail for unrelated network/auth reasons instead. Requiring the
  # exact provenance marker above proves the script rejected the origin before
  # trusting/fetching origin/main or invoking the deployment script.
