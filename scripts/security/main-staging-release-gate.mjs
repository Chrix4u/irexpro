import { requiredWorkflowNames } from './required-ci-gate.mjs';
...
export function runSelfTests() {
  ...
  assertEqual(
    ...
    'staging CD workflow policy',
  );
  assertEqual(
    requiredWorkflowNames(['scripts/security/main-staging-release-gate.mjs']),
    ['Release Security', 'Deployment Script Safety'],
...
export async function runMainStagingReleaseGate() {
  ...
  const pollSeconds = positiveIntegerEnvironment(
    'MAIN_RELEASE_POLL_SECONDS',
    DEFAULT_POLL_SECONDS,
  );
  const timeoutSeconds = positiveIntegerEnvironment(
    'MAIN_RELEASE_TIMEOUT_SECONDS',
  ...
  const beforeSha = await resolveBeforeSha(
    repository,
    candidateSha,
    token,
    apiUrl,
