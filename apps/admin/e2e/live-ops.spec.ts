  ...
  activeControls: [
    {
      ...
      scopeTarget: null,
      reason:
        "Investigating provider reconciliation drift across LIVE connections",
...
function connectionRow(i: number): AdminConnectionRowView {
      ...
      : i === 2
        ...
        : i === 8
          ...
          : i % 4 === 0
            ? "ACTIVE"
            : "AUTHORIZED";
  const credentialStatus = isError
    ? "INVALID"
    : i === 2
      ? "REVOKED"
      : "VERIFIED";
  ...
  return {
    ...
    lastSyncAt:
      i % 7 === 0
        ? null
        : `2025-07-${String((i % 28) + 1).padStart(2, "0")}T10:00:00.000Z`,
    lastHealthCheckAt:
      i % 7 === 0
        ? null
        : `2025-07-${String((i % 28) + 1).padStart(2, "0")}T10:00:00.000Z`,
...
const connectionRows: AdminConnectionRowView[] = Array.from(
  { length: 30 },
  (_, i) => connectionRow(i),
);
...
const discrepancyRows: AdminDiscrepancyRowView[] = DISCREPANCY_TYPES.map(
  (meta, i) => {
    const n = String(i + 1).padStart(2, "0");
    ...
    return {
      ...
      resolutionNote: resolved
        ? "Provider-authoritative close adopted by the reconciliation job."
        : null,
    };
  },
);
...
function filterAuditLogs(
  ...
  let logs = rows;
  if (filter === "CRITICAL")
    logs = logs.filter((row) => row.severity === "CRITICAL");
  if (filter === "WARNING")
    logs = logs.filter((row) => row.severity === "WARNING");
  if (actorUserId) logs = logs.filter((row) => row.actorUserId === actorUserId);
  if (resourceType)
    logs = logs.filter((row) => row.resourceType === resourceType);
  return logs;
}
...
function statValue(page: Page, label: string): Locator {
  return page
    .locator(".stat-card")
    .filter({
      has: page.locator(".stat-card__label", {
...
test.describe("Admin Live Ops overview page", () => {
  test("connection-state and discrepancy stat tiles render contract numbers", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Generated 2025-07-15 09:30 UTC/),
    ).toBeVisible();
    await expect(
  ...
  test("active execution controls list shows GLOBAL and scoped controls", async ({
    page,
  }) => {
    await gotoLiveOpsPage(page, "/admin/live-ops", /^live ops$/i);
    ...
    await expect(globalCard).toContainText(
      "Investigating provider reconciliation drift",
    );
    ...
    await expect(scopedCard.locator(".badge--warning")).toHaveText(
      /broker connection/i,
    );
    await expect(scopedCard).toContainText(
      "conn_00000000-0000-0000-0000-000000000010",
    );
  ...
  test("provider capability matrix renders brokers, environments, capability chips and +N more", async ({
    page,
  }) => {
    await gotoLiveOpsPage(page, "/admin/live-ops", /^live ops$/i);
  ...
  test("shows the empty state when no emergency controls are active", async ({
    page,
  }) => {
    await gotoLiveOpsPage(page, "/admin/live-ops", /^live ops$/i);
    ...
    await page.route("**/api/v1/admin/live-account/overview**", (route) =>
      route.fulfill(
        jsonFulfill(200, { ...overviewFixture, activeControls: [] }),
      ),
    );
    await page.reload();
    await expect(
      page.getByRole("heading", { level: 1, name: /^live ops$/i }),
    ).toBeVisible();
  ...
  test("discrepancy log renders severity and status badges", async ({
    page,
  }) => {
    await gotoLiveOpsPage(page, "/admin/live-ops", /^live ops$/i);

    const table = page.locator(
      'table[aria-label="Reconciliation discrepancies"]',
    );
    await expect(table.locator("tbody tr")).toHaveCount(10);
    // First row is the CRITICAL OPEN fixture.
    await expect(
      table.locator("tbody tr").first().locator(".badge--error"),
    ).toHaveText("CRITICAL");
    await expect(
  ...
  test("discrepancy filter CRITICAL refetches with the query param and filters rows", async ({
    ...
    const capture = await gotoLiveOpsPage(
      page,
      "/admin/live-ops",
      /^live ops$/i,
    );
    ...
    expect(
      capture.discrepancies.some(
        (url) => readQuery(url).get("filter") === "CRITICAL",
      ),
      `Expected a discrepancies request with filter=CRITICAL, got: ${capture.discrepancies.join(", ")}`,
    ).toBe(true);
  });

  test("discrepancy pagination advances the offset and swaps rows", async ({
    page,
  }) => {
    const capture = await gotoLiveOpsPage(
      page,
    ...
    expect(
      capture.discrepancies.some(
        (url) => readQuery(url).get("offset") === "10",
      ),
  ...
  test("refresh button refetches the overview", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Refresh live operations overview" })
      .click();
    await expect
  ...
  test("Live Ops navigation entry is reachable", async ({ page }) => {
    ...
    if (viewport && viewport.width > 700) {
      ...
      const sheetItem = page.locator(".mobile-sheet__item", {
        hasText: "Live ops",
      });
  ...
  test("filter buttons meet 44px touch targets on mobile", async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(
        box.height,
        `Filter button ${i} height=${box.height} < 44`,
...
test.describe("Admin Brokers connections page", () => {
  test("renders connection rows, state badges, and the fail-closed execution gate", async ({
    ...
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 02" }),
    ).toContainText("Account 02");
    ...
    // shows DEMO.
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 07" }),
    ).toContainText("LIVE");
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 02" }),
    ).toContainText("DEMO");
  ...
  test("connection filter ERROR refetches with the query param and shows only error rows", async ({
    ...
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 06" }),
    ).toBeVisible();
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 21" }),
    ).toBeVisible();
    await expect(page.getByText(/Showing 1–2 of 2/)).toBeVisible();

    expect(
      capture.connections.some(
        (url) => readQuery(url).get("filter") === "ERROR",
      ),
      `Expected a connections request with filter=ERROR, got: ${capture.connections.join(", ")}`,
    ).toBe(true);
  });

  test("connection pagination advances the offset and swaps rows", async ({
    page,
  }) => {
    const capture = await gotoLiveOpsPage(page, "/admin/brokers", /^brokers$/i);
    ...
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 26" }),
    ).toBeVisible();
    // Page-1-only row is gone.
    await expect(
      table.locator("tbody tr", { hasText: "MT5 Broker 01" }),
    ).toHaveCount(0);
  ...
  test("page does not horizontally overflow", async ({ page }) => {
    await gotoLiveOpsPage(page, "/admin/brokers", /^brokers$/i);
    await expect(
      page.locator('table[aria-label="Broker connections"] tbody tr').first(),
    ).toBeVisible();
    await assertNoHorizontalOverflow(page);
  });
...
test.describe("Admin Audit page", () => {
  test("renders audit rows with severity badges and masked actor ids", async ({
    page,
  }) => {
    ...
    await expect(table.locator("tbody .badge--error").first()).toHaveText(
      "CRITICAL",
    );
    await expect(table.locator("tbody .badge--warning").first()).toHaveText(
      "WARNING",
    );
    await expect(table.locator("tbody .badge--info").first()).toHaveText(
      "INFO",
    );

    // Action + correlation render in the monospace cell style.
    await expect(
      table.locator("tbody .admin-table__cell-mono").first(),
    ).toBeVisible();
    await expect(table.getByText("ORDER_SUBMITTED").first()).toBeVisible();
    await expect(
      table.getByText("EXECUTION_CONTROL_ACTIVATED").first(),
    ).toBeVisible();
  ...
  test("audit pagination advances the offset and swaps rows", async ({
    page,
  }) => {
    const capture = await gotoLiveOpsPage(page, "/admin/audit", /^audit log$/i);
  ...
  test("page does not horizontally overflow", async ({ page }) => {
    await gotoLiveOpsPage(page, "/admin/audit", /^audit log$/i);
    await expect(
      page.locator('table[aria-label="Audit records"] tbody tr').first(),
    ).toBeVisible();
    await assertNoHorizontalOverflow(page);
  });
