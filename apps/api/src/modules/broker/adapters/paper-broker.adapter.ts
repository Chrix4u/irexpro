...
export class PaperBrokerAdapter implements IBrokerAdapter {
  ...
  /**
   * Runtime connection-scoped PAPER adapters receive a durable store plus
   * BrokerConnection id. Root/ephemeral adapters intentionally remain
   * isolated in-memory instances for metadata/credential tests.
   */
  constructor(
    ...
    try {
      ...
      if (stored) {
        ...
        if (bootstrap) {
          ...
          this._orderCounter = Math.max(this._orderCounter, bootstrap.maxPaperOrderCounter);
        }
  ...
  async connect(_credentials: DecryptedBrokerCredentials): Promise<BrokerConnectionResult> {
    ...
    // Restore durable provider truth before exposing the connection as healthy.
